...
/**
 * Builds copied atlas asset data for a view file.
 */
function buildInlineAssetData(assetEntry, atlasPngUrl) {
    const data = {
        name: String(assetEntry.name || ""),
        url: atlasPngUrl,
        frameName: String(assetEntry.frameName || ""),
 ...
 */
function buildViewAssetConstants(view, atlasPngUrl, constName) {
    ...
    getClassViewChildren(view)
        ...
        .forEach((child) => {
            ...
            assetDataByName[String(child.name || assetEntry.name || "")] = {
                asset: buildInlineAssetData(assetEntry, atlasPngUrl),
                x: Number(child.x || 0),
                y: Number(child.y || 0),
...
function buildClassAssetChildLines(props) {
    ...
    return [
        `    const ${dataVarName} = ${assetConstName}[${childKey}];`,
        `    const ${assetVarName} = ${dataVarName}.asset;`,
        `    const ${childVarName} = new Phaser.GameObjects.Image(scene, 0, 0, ${assetVarName}.url, ${assetVarName}.frameName);`,
        `    ${childVarName}.name = ${assetVarName}.name || "";`,
        `    setLeftTop(${childVarName}, ${dataVarName}.x - ${viewWidthRef} / 2, ${dataVarName}.y - ${viewHeightRef} / 2);`,
        `    this.add(${childVarName});`,
    ].join("\n");
...
function buildViewClassFile(props) {
    ...
    const assetConstants = buildViewAssetConstants(view, atlasPngUrl, assetConstName);
    ...
    const factoryName = buildRootFactoryName(view);
    const factoryFunction = `export function ${factoryName}(props: ${propsInterfaceName}): ${className} {\n  return new ${className}(props);\n}\n`;
...
function buildViewIndexTs(viewEntries, classNameMap) {
    const exports = viewEntries
        .map((entry) => {
            ...
            if (!className) return "";
            return `export { ${className}, ${buildRootFactoryName(entry)} } from "./${className}";`;
        })
        .filter(Boolean)
        .join("\n");
