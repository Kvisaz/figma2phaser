...
- предупреждения по именам, небезопасным для файловой системы.
...
Если имя корневого узла или верхнего ребенка содержит что-то кроме латиницы, цифр, `.`, `_`, `-`, плагин показывает предупреждение в UI и пишет список в `manifest.warnings.unsafeNames`.
Экспорт при этом продолжается: итоговые file names по-прежнему создаются через безопасный `slugify`.
...
В `ui.html` реализован `MaxRects`-пэкер:
- перед упаковкой PNG обрезаются по alpha-bounds, а исходные размеры сохраняются в atlas JSON;
- изображения сортируются от крупных к мелким;
- для каждого изображения перебираются normal/rotated варианты;
- packing прогоняется несколькими эвристиками MaxRects;
- размер atlas подбирается из кандидатов и ужимается до реально занятой области;
- `padding = 2` между frame;
- в UI показывается заполненность atlas, количество rotated/trimmed frames и экономия trim.

Старый `shelf`-пэкер оставлен как fallback, если MaxRects не смог упаковать набор в пределах `maxAtlasSize`.
Пэкер работает без внешних Node-зависимостей.
    ...
    "unsafeNames": [
      {
        ...
        "name": "кнопка старт",
        "safeName": "item",
        "invalidCharacters": ["к", "н", "о", "п", "а", " ", "с", "т", "р"]
      }
    ]
  }
}
